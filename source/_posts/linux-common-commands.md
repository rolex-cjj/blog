...
touch、mkdir、cp、mv(相同目录下使用为重命名，不同目录下为剪切)、rm(rm -rf一定要少用!!!深受其害!!! ~~o(>_<)o ~~)、file(查看文件类型)、pwd(查看当前路径)、ls(ls -l -a -h)、whoami(目前身份)、
...
cat(一次性显示)、tac、more(向下翻页)、less(可上下翻页)、head(head -n number file)、tail、od(非纯文本文件)
...
![](http://ohjnxvaxm.bkt.clouddn.com/variable-operation.png)
...
cut(同一行里面的数据进行分解)、grep(取出有关键字的行grep -n)、sed、awk(处理每一行字段内的数据 )、
...
tr、col、join(两文档以数据相关性贴在一起)、paste(将两行贴在一起)、split(大档案分割为小档案)
...
![](http://ohjnxvaxm.bkt.clouddn.com/regular-ba.png)
...
free(内存使用情况)、lsb_release -a(显示操作系统版本信息，Linux Standard Base缩写lsb，意为linux标准规范库)、uname -a(显示当前操作系统与内核信息)、netstat(跟踪网络)、/proc/*、
